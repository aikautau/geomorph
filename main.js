...
function populateFilters(gj){
  ...
  els.type.value=state.type; els.pref.value=state.pref; els.q.value=state.q;
  ...
  els.clear.addEventListener('click', ()=>{ state.type=""; state.pref=""; state.q=""; els.type.value=""; els.pref.value=""; els.q.value=""; update(); });
  els.share.addEventListener('click', ()=>{
    const h = new URLSearchParams({type:state.type,pref:state.pref,q:state.q}).toString();
    location.hash = h;
    if (navigator.clipboard) navigator.clipboard.writeText(location.href);
    els.share.textContent = "コピーしました";
    setTimeout(()=>els.share.textContent="共有",1500);
  });
}
